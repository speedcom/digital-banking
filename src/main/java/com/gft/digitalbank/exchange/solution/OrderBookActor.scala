...
class OrderBookActor(exchangeActorRef: ActorRef, product: String) extends Actor {
  ...
  override def receive: Receive = {
    case BuyOrder(b)    => orderBook.addBuyOrder(OrderBookValue(b))
    case SellOrder(s)   => orderBook.addSellOrder(OrderBookValue(s))
    case CancelOrder(c) => orderBook.cancelOrder(c)
    case ModifyOrder(m) => orderBook.modifyOrder(m)
